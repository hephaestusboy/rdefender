...
def load_models():
    return {
        "rf_b":   joblib.load("rf_behavior_model_v5.joblib"),
        "rf_a":   joblib.load("rf_artifact_model_v5.joblib"),
        "xgb_b":  joblib.load("xgb_behavior_model_v5.joblib"),
        "xgb_a":  joblib.load("xgb_artifact_model_v5.joblib"),
        "lgbm_b": joblib.load("lgbm_behavior_model_v5.joblib"),
        "lgbm_a": joblib.load("lgbm_artifact_model_v5.joblib"),
        "cat_b":  joblib.load("catboost_behavior_model_v5.joblib"),
        "cat_a":  joblib.load("catboost_artifact_model_v5.joblib"),
        "fusion": joblib.load("fusion_model_v5.joblib"),
    }
...
# =========================================================
# 32-FEATURE FUSION ARRAY  (must match train_ensemble_v5.py _build_meta)
#
...
# =========================================================
def build_fusion_features(p_rb, p_ra, p_xb, p_xa, p_lb, p_la, p_cb, p_ca, raw_vec):
...
def predict_file(models, filepath):
    ...
    p_rb = models["rf_b"].predict_proba(vec1)[0][1]
    p_ra = models["rf_a"].predict_proba(vec2)[0][1]
    p_xb = models["xgb_b"].predict_proba(vec1)[0][1]
    p_xa = models["xgb_a"].predict_proba(vec2)[0][1]
    p_lb = models["lgbm_b"].predict_proba(vec1)[0][1]
    p_la = models["lgbm_a"].predict_proba(vec2)[0][1]
    p_cb = models["cat_b"].predict_proba(vec1)[0][1]
    p_ca = models["cat_a"].predict_proba(vec2)[0][1]

    fusion_input = build_fusion_features(p_rb, p_ra, p_xb, p_xa, p_lb, p_la, p_cb, p_ca, raw_vec)
    ...
    if fusion_prob >= MALWARE_THRESHOLD:
        ...
        # consensus: majority of base models agree
        consensus_suspicion = sum([
            p_rb > 0.30, p_ra > 0.30, p_xb > 0.30, p_xa > 0.30,
            p_lb > 0.30, p_la > 0.30, p_cb > 0.30, p_ca > 0.30
        ]) >= 6

        # shadow_del override requires meaningful model confidence to avoid low-signal FPs
    ...
    if label == "RANSOMWARE" and raw_signed == 1 and fusion_prob < 0.98:
        label = "SUSPICIOUS"
    ...
    audit_info = {
        ...
        "pb_xgb":       round(p_xb, 4),
        "pa_xgb":       round(p_xa, 4),
        "pb_lgbm":      round(p_lb, 4),
        "pa_lgbm":      round(p_la, 4),
        "pb_cat":       round(p_cb, 4),
        "pa_cat":       round(p_ca, 4),
...
if __name__ == "__main__":
    ...
    for path, label, prob, audit_info, error in results_ben:
        ...
        if label == "BENIGN":
            ...
            if label == "SUSPICIOUS": susp_ben += 1
    ...
    print()
    print("═" * W)
    print(f"  R-DEFENDER  ENSEMBLE v5  —  MODEL PERFORMANCE REPORT")
    ...
    if best_rec >= RECALL_TARGET:
        ...
        print(f"     → python3 train_ensemble_v5.py --retarget")
    ...
    if fn_audit_log or fp_audit_log:
        ...
        if cluster_fp.get("consensus-fp"):   print(f"    • consensus FPs      ({cluster_fp['consensus-fp']}): majority wrong → add to benign training → --retarget")
    ...
    if fn_audit_log:
        fn_path = f"fn_audit_v5_{timestamp}.csv"
    ...
    if fp_audit_log:
        fp_path = f"fp_audit_v5_{timestamp}.csv"
