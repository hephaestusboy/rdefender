...
def extract_single_file(file_info):
    """Worker function for independent process extraction."""
...
def build_catboost(y):
    ...
    return cbt.CatBoostClassifier(
        ...
        thread_count=half_cores,
        random_state=42,
        verbose=0
    )

# =========================================================
# MULTICORE FUSION TASK (HYBRID 12-FEATURE ARRAY)
# =========================================================

def train_fold(fold_info, X, X1, X2, y):
    """Worker function for parallel K-Fold training with Interaction Features."""
    fold_idx, (train_idx, val_idx) = fold_info
    
    # Local calibration within the fold
    lgbm_b = CalibratedClassifierCV(build_lgbm(y[train_idx]), method="isotonic", cv=3)
    lgbm_a = CalibratedClassifierCV(build_lgbm(y[train_idx]), method="isotonic", cv=3)
    cat_b = CalibratedClassifierCV(build_catboost(y[train_idx]), method="isotonic", cv=3)
    cat_a = CalibratedClassifierCV(build_catboost(y[train_idx]), method="isotonic", cv=3)

    lgbm_b.fit(X1[train_idx], y[train_idx])
    lgbm_a.fit(X2[train_idx], y[train_idx])
    cat_b.fit(X1[train_idx], y[train_idx])
    cat_a.fit(X2[train_idx], y[train_idx])
    ...
    probs = np.vstack([p_lgbm_b, p_lgbm_a, p_cat_b, p_cat_a]).T
    ...
    # Signal-based "Silver Bullets"
    raw_silver = X[val_idx][:, SILVER_INDICES]
    art_dominance = np.maximum(0.0, avg_artifact - avg_behavior)
    max_signal = np.max(probs, axis=1)
    fuzzy_anomaly = raw_silver[:, 2] * (raw_silver[:, 3] / 8.0) 
    file_entropy = raw_silver[:, 3]
    # In your training script's train_fold function:
    # This helps the model realize that "Conflict" is often a sign of stealth
    high_artifact_stealth = (avg_artifact > 0.7) & (avg_behavior < 0.2)
    high_behavior_stealth = (avg_behavior > 0.7) & (avg_artifact < 0.2)

    # Extract all useful signals without worrying about the count
    meta = np.column_stack([
        # 1-4: Raw Probs
        p_lgbm_b, p_lgbm_a, p_cat_b, p_cat_a, 
        # 5-8: Averages & Conflict
        avg_behavior, avg_artifact, disagreement, prob_entropy,
        # 9-12: Stealth Interaction Flags
        high_artifact_stealth.astype(float),
        high_behavior_stealth.astype(float),
        (avg_artifact * avg_behavior), # Joint confidence
        # 13-16: Model-Specific Dominance
        (p_lgbm_a - p_lgbm_b), (p_cat_a - p_cat_b),
        np.max(probs, axis=1), np.min(probs, axis=1),
        # 17-20: All Silver Bullets
...
if __name__ == "__main__":
    ...
    print("=========================================================")
    print(" 🛠️ R-DEFENDER MULTICORE PIPELINE (HYBRID STACKING) - CATBOOST + LIGHTGBM")
    print("=========================================================")

    # 1. Multicore Extraction
    ...
    # 4. Meta-Learner (Hybrid 12-Feature Array)
    print("🔗 Training Hybrid 12-Feature Meta-Learner...")
    pos = (fusion_y == 1).sum()
    neg = (fusion_y == 0).sum()

    # Using CatBoost as the meta-learner
    cat_meta = cbt.CatBoostClassifier(
        n_estimators=600,
        max_depth=6,
        learning_rate=0.02,
    ...
    joblib.dump(fusion_model, "catlgbm_fusion_model.joblib")
