...
def train_fold(fold_info, X, X1, X2, y):
    ...
    rf_b = CalibratedClassifierCV(build_rf(y[train_idx]), method="isotonic", cv=3)
    rf_a = CalibratedClassifierCV(build_rf(y[train_idx]), method="isotonic", cv=3)
    xgb_b = CalibratedClassifierCV(build_xgb(y[train_idx]), method="isotonic", cv=3)
    xgb_a = CalibratedClassifierCV(build_xgb(y[train_idx]), method="isotonic", cv=3)
...
if __name__ == "__main__":
    ...
    print("=========================================================")
    print(" 🛠️ R-DEFENDER MULTICORE PIPELINE (HYBRID STACKING)")
    print("=========================================================")
    ...
    print("\n🔗 Training Hybrid 12-Feature Meta-Learner...")
    pos = (fusion_y == 1).sum()
    neg = (fusion_y == 0).sum()

    # In your training script
    xgb_meta = XGBClassifier(
        n_estimators=600,       # More estimators to handle the complex 19-feature space
        max_depth=6,            # Depth 6 is usually the sweet spot for 19-24 features
        learning_rate=0.02,
        scale_pos_weight=3.0,   # Increased penalty for False Negatives
        subsample=0.8,
