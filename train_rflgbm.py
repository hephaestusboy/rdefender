...
def train_fold(fold_info, X, X1, X2, y):
    ...
    probs = np.vstack([p_rf_b, p_rf_a, p_lgbm_b, p_lgbm_a]).T
...
def build_fusion_dataset_multicore(X, X1, X2, y):
    ...
    print(f"\n🧠 Building RF+LGBM Hybrid Fusion Dataset Parallelly ({len(folds)} Folds)...")
...
if __name__ == "__main__":
    ...
    print("=========================================================")
    print(" 🛠️ R-DEFENDER MULTICORE PIPELINE (RF + LGBM)")
    print("=========================================================")

    if os.path.exists("cached_dataset_rflgbm.npz"):
        data = np.load("cached_dataset_rflgbm.npz")
        X, y = data["X"], data["y"]
        print("Dataset loaded from cache.")
    else:
        X, y = build_dataset_incremental(mal_files, ben_files)
        np.savez_compressed("cached_dataset_rflgbm.npz", X=X, y=y)
    ...
    xgb_meta = XGBClassifier(
        n_estimators=600, max_depth=6, learning_rate=0.02, scale_pos_weight=3.0,
